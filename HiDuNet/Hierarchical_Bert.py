...
def set_seed(args):
    ...
    # Also seed CUDA (if available) for reproducibility
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Use named output where possible (SequenceClassifierOutput)
            loss = getattr(outputs, 'loss', None)
            if loss is None:
                # fallback to legacy positional index
                loss = outputs[0]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                try:
                    ...
                    # Some schedulers require a step per batch; keep robust
                    pass
                model.zero_grad()
                global_step += 1
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        if results[metric_key] > best:
                            ...
                            logger.info("Saving optimizer and scheduler state to %s", output_dir)
                    ...
                    # Prefer get_last_lr for modern schedulers
                    try:
                        learning_rate_scalar = scheduler.get_last_lr()[0]
                    except Exception:
                        learning_rate_scalar = None
    ...
    logs = {}
    if (args.evaluate_during_training and args.logging_steps > 0 and global_step % args.logging_steps != 0 and keep_best_step < args.early_stop_steps):
        results = evaluate(args, model, tokenizer, 'dev')
        res_for_display = {}
        for k, v in results.items():
            res_for_display[k.replace('-', '_')] = v
...
def main():
    ...
    if args.do_infer:
        ...
        if os.path.exists(best_model_path):
            model = ElasticBertForSequenceClassification.from_pretrained(best_model_path, args = args)
            model.to(args.device)
            results = evaluate(args, model, tokenizer, 'test')
